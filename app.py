...
# 測試離線翻譯頁面：提供一個 HTML 表單，利用 JavaScript 發送 JSON 請求
...
def basic_translate_test():
    ...
    <p>注意：請使用 JSON 格式的 POST 請求測試 API。</p>
    '''

# 測試語音生成頁面：提供一個 HTML 表單，利用 JavaScript 發送 JSON 請求，並返回音訊檔
...
def text_to_speech_test():
    ...
    <p>注意：請使用 JSON 格式的 POST 請求生成語音。</p>
    '''

# 測試日誌更新頁面：提供一個 HTML 表單，利用 JavaScript 發送 JSON 請求更新日誌
...
def log_task_test():
    ...
    <p>注意：請使用 JSON 格式的 POST 請求更新日誌。</p>
    '''

# 新增測試影片剪輯頁面：提供一個 HTML 表單，利用 JavaScript 發送 JSON 請求測試影片剪輯功能
...
def edit_video_test():
    if request.method == 'POST':
        ...
        success = clip_video(input_video, output_video, start_time, end_time)
        if success:
            return jsonify({"message": "影片剪輯完成", "output": output_video})
        else:
            return jsonify({"error": "影片剪輯失敗"}), 500
    else:
        return '''
        <h3>影片剪輯測試</h3>
        <p>請使用 JSON 格式的 POST 請求來測試影片剪輯功能。</p>
        <ul>
            <li><strong>input_video</strong>：原始影片完整路徑</li>
            <li><strong>output_video</strong>：剪輯後影片完整路徑</li>
...
if __name__ == '__main__':
    # 開發環境使用；如需使用 HTTPS 可加入 ssl_context=('cert.pem', 'key.pem')
